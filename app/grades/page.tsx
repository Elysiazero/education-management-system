...
// 在组件中添加新的状态
  ...
  const handleGenerateRecommendations = async () => {
    ...
    try {
      ...
      // 获取学生数据
      const student = analytics.find(a => a.studentId === selectedStudentId);
      if (!student) return;

      const studentGrades = grades.filter(g => g.studentId === selectedStudentId);

      // 生成基础建议
      let generatedRecs = generatePersonalizedRecommendations(selectedStudentId);

      // 根据关注领域过滤
      if (focusArea !== "all") {
        generatedRecs = generatedRecs.filter(rec => {
          if (focusArea === "knowledge") {
            return rec.includes("知识") || rec.includes("基础") || rec.includes("理解");
          }
          if (focusArea === "skills") {
            return rec.includes("技能") || rec.includes("能力") || rec.includes("实践");
          }
          if (focusArea === "habits") {
            return rec.includes("习惯") || rec.includes("时间") || rec.includes("效率");
          }
          if (focusArea === "engagement") {
            return rec.includes("参与") || rec.includes("互动") || rec.includes("沟通");
          }
          return true;
        });
      }
      ...
      // 转换为结构化格式
// 修改 generatePersonalizedRecommendations 返回的结构
      const structuredRecs = generatedRecs.map(rec => {
        let priority: "high" | "medium" | "low" = "medium";
        if (rec.includes("立即") || rec.includes("特别关注") || rec.includes("需要")) {
          priority = "high";
        } else if (rec.includes("可以") || rec.includes("建议") || rec.includes("考虑")) {
          priority = "low";
        }

        // 确保 resources 始终是数组
        let resources: string[] = [];
        if (rec.includes("基础知识")) {
          resources = ["基础知识点总结.pdf", "概念讲解视频"];
        } else if (rec.includes("实验")) {
          resources = ["实验操作指南", "优秀实验报告范例"];
        } else if (rec.includes("项目")) {
          resources = ["项目管理模板", "团队协作技巧"];
        }
          ...
          ({Math.round((grade.finalScore / 100) * 100)}%)
              ...
              <TabsContent value="recommendations">
                <Card>
                  ...
                  <CardContent>
                    {analytics.length > 0 && analytics[0].recommendations.length > 0 ? (
                        <div className="space-y-4">
                          {analytics[0].recommendations.map((recommendation, index) => (
                              <div key={index} className="p-4 bg-blue-50 rounded-lg border-l-4 border-blue-500 flex items-start">
